...
namespace BYOD_SalesCube_CustomerInvoiceLines
{
  class Mail
  {
    ...
    public Boolean SendMail(String template, MemoryStream ms2, String ms3)
    {
      ...
      try
      {
        ...
        mail.Subject = "Cotizaciones abiertas mayores a 20k";
        mail.Body = "<p>Esta usted recibiendo un archivo adjunto con la lista de cotizaciones abiertas que tienen un monto total mayor a $20,000.<p>";
        ...
        foreach (String emailCliente in listaCorreos)
        {
          if (IsValidEmail(emailCliente))
          {
            ...
            //mail.To.Add(new MailAddress("sistemas09@avanceytec.com.mx", "sistemas09@avanceytec.com.mx", Encoding.UTF8));
          }
        }
        ...
        attach.ContentDisposition.FileName = "Cotizaciones Abiertas mayores 20k (" + DateTime.Now.ToString("yyyy-MM-dd") + ").pdf";
        ...
        ctExcel.CharSet = Encoding.Unicode.EncodingName;
        //System.Net.Mime.ContentType ctExcel = new System.Net.Mime.ContentType(System.Net.Mime.MediaTypeNames.Application.Octet);
        System.Net.Mail.Attachment attachExcel = new System.Net.Mail.Attachment(new MemoryStream(Encoding.ASCII.GetBytes(ms3)), ctExcel);
        attachExcel.ContentDisposition.FileName = "Cotizaciones Abiertas mayores 20k (" + DateTime.Now.ToString("yyyy-MM-dd") + ").xls";
        attachExcel.ContentType.CharSet = Encoding.UTF8.EncodingName;
        attachExcel.ContentType = new System.Net.Mime.ContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");

        try
        {
          mail.From = new MailAddress("notificacioninterna@avanceytec.com.mx", cuentaAlias);
          //mail.Attachments.Add(attach);
          mail.Attachments.Add(attachExcel);
          client.Send(mail);
        }
